...
import { Plus, Pencil, Trash2, X, ShoppingBag, ExternalLink, Eye, EyeOff, Lock } from "lucide-react";
...
export function BarbershopAdvertisementsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingAd) {
        await axios.put(`${API}/advertisements/barbershop/${editingAd.id}`, payload, { withCredentials: true });
        toast.success("Propaganda atualizada!");
      } else {
        await axios.post(`${API}/advertisements/barbershop`, payload, { withCredentials: true });
        toast.success("Propaganda criada!");
      }
      setShowForm(false);
      loadAds();
    } catch (e) {
      toast.error(e?.response?.data?.detail || "Erro ao salvar propaganda");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggle = async (ad) => {
    try {
      await axios.put(`${API}/advertisements/barbershop/${ad.id}`, { is_active: !ad.is_active }, { withCredentials: true });
  ...
  const handleDelete = async (ad) => {
    ...
    try {
      await axios.delete(`${API}/advertisements/barbershop/${ad.id}`, { withCredentials: true });
  ...
  const renderAdCard = (ad, isCustom) => (
    <Card key={ad.id} className={`bg-surface border-border p-4 ${!ad.is_active ? "opacity-50" : ""}`}>
      <div className="flex items-start justify-between mb-3">
        ...
        {!isCustom && (
          <Badge className="text-xs bg-blue-900 text-blue-300 border-blue-700 border shrink-0 ml-2">
            <Lock className="w-3 h-3 mr-1" />Global
          </Badge>
        )}
        {isCustom && (
          <Badge className={`text-xs ${ad.is_active ? "bg-green-900 text-green-300 border-green-700" : "bg-zinc-800 text-zinc-400 border-zinc-600"} border shrink-0 ml-2`}>
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          {/* Custom ads */}
          <div className="mb-8">
            <h2 className="text-lg font-semibold text-white mb-3">Suas Propagandas</h2>
            {customAds.length === 0 ? (
              <Card className="p-8 text-center bg-surface border-border">
                <ShoppingBag className="w-10 h-10 text-zinc-600 mx-auto mb-3" />
                <p className="text-zinc-400 text-sm">Você ainda não cadastrou propagandas próprias.</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {customAds.map((ad) => renderAdCard(ad, true))}
              </div>
            )}
          </div>
          ...
          {globalAds.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold text-white mb-1">Propagandas Globais</h2>
              <p className="text-xs text-zinc-500 mb-3">Definidas pelo administrador — aparecem para todos. Você não pode editar.</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {globalAds.map((ad) => renderAdCard(ad, false))}
              </div>
            </div>
          )}
        </>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <Card className="bg-surface border-border w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
